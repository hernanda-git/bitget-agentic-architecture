...
from src.execution.fake_exchange import FakeExchange, OrderRequest
...
def _empty(): return {"closed_trades": 0, "fees": 0.0, "funding": 0.0, "net_pnl": 0.0}
...
def run_baseline(snapshots: Iterable, config: BaselineConfig = BaselineConfig()) -> BaselineResult:
    ...
    total_fees = total_funding = total_net = 0.0; closed = orders = 0
    replay_parts = []
    for index, snapshot in enumerate(snapshots):
        ...
        for name, generator in generators:
            ...
            total_fees += trade["entry_fee"] + trade["exit_fee"]; total_funding += funding
            total_net += trade["net_pnl"] - funding; closed += 1
            row = strategy[name]; row["closed_trades"] += 1; row["fees"] += trade["entry_fee"] + trade["exit_fee"]; row["funding"] += funding; row["net_pnl"] += trade["net_pnl"] - funding
            regime_name = classify_regime(snapshot).value; rr = regime[regime_name]; rr["closed_trades"] += 1; rr["fees"] += trade["entry_fee"] + trade["exit_fee"]; rr["net_pnl"] += trade["net_pnl"] - funding
    import hashlib, json
    ...
    return BaselineResult(len(snapshots), 0, 0, orders, closed, total_fees, total_funding, total_net,
