"""Purged chronological walk-forward evaluation for the order-flow impulse candidate.

Extends the purged holding-period label infrastructure from Phase 62
(evaluate_purged_holding_periods) to the walk-forward boundary. When
evaluating the order-flow impulse strategy across walk-forward windows,
...
class PurgedOrderflowWindowResult:
...
def _walk_forward_windows(n: int, config) -> list[dict]:
    ...
    while test_start + window <= n:
        ...
        windows.append({"train_start": 0, "train_end": test_start - embargo - 1,
                        "test_start": test_start, "test_end": test_end})
        test_start = test_end + 1 + embargo
    return windows


def evaluate_purged_orderflow_walk_forward(
    closes: list[float],
    period: int,
    *,
    ...
    for win_idx, win in enumerate(windows):
        ...
        # Purged: labels only where i + period <= test_end
        # and i >= test_start
...
class _WalkForwardConfig:
    ...
    def __init__(self, train_fraction, test_window, embargo):
