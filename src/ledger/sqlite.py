...
class EventLedger:
    ...
    def append(self, event_type: str, payload: dict) -> int:
        """Legacy facade that still builds a complete validated RuntimeEvent."""
        ...
        value = {"event_type": event_type, "cycle_id": cycle_id, "trace_id": str(payload.get("trace_id") or f"trace-{cycle_id}"),
                 ...
                 "symbol": str(payload.get("symbol") or "UNKNOWN"), "payload": payload}
    ...
    def realized_pnl(self): return sum(float(json.loads(r["event_json"]).get("realized_pnl", 0)) for r in self._rows("fills"))
    def fees(self): return sum(float(json.loads(r["event_json"]).get("fee", 0)) for r in self._rows("fills"))
    def funding(self): return sum(float(json.loads(r["event_json"]).get("funding", 0)) for r in self._rows("fills"))
